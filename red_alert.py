...
class RedAlert():
    ...
    def get_cities(self, hebrew_cities:list):
        ...
        for city in hebrew_cities:
            search = self.binary_search(self.cities, city)
            if search is None:
    ...
    def get_polygons(self, ids):
        ...
        polygons = []
        for id in ids:
            search = self.binary_search(self.polygons, id)
            if search is not None:
                polygons.append(search)
        return polygons

    def binary_search(self, json_obj, target_key):
        "binary search, the quickest way to search sorted arrays :D"
        target_key = str(target_key)
        keys = sorted(json_obj.keys())
